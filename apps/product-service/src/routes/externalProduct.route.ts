...
router.post("/import", shouldBeAdmin, async (req: Request, res: Response) => {
  try {
    ...
    if (
      ...
      return res
        .status(400)
        .json({ error: "Stock quantity must be 0 or more" });
    }
    ...
    if (!existingCategory) {
      return res
        .status(400)
        .json({ error: `Invalid category slug: ${categorySlug}` });
    }
    ...
    const product = await prisma.product.create({
      data: {
        ...
        shortDescription:
          externalProduct.description?.substring(0, 200) ||
    ...
    console.log(
      `Product imported from ${externalProduct.source}: ${product.id} - ${product.name}`,
    );
...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (
        !source ||
        !["techspecs", "fakestore", "dummyjson", "platzi"].includes(source)
      ) {
        return res.status(400).json({
          error:
            "Invalid source. Must be 'techspecs', 'dummyjson', 'platzi', or 'fakestore'",
        });
      }
